 ...
 *
 * Rendering + effects only. Upload protocol lives in
 * `domain/blobUpload.ts` / `adapters/driven/blobUploadIo.ts`, attachment crypto
 * in `domain/attachmentCrypto.ts`, and blob download in
 * `adapters/driven/attachmentBlobIo.ts`.
 */
import { useCallback, useEffect, useRef, useState } from 'react';
import { MAX_ATTACHMENTS_PER_MESSAGE, parseAttachmentRefs, validateFile, type AttachmentRef } from '../../../domain/attachments.mjs';
import { encryptAttachment, type AllowedMime, type PreparedAttachment } from '../../../domain/attachmentCrypto';
import { formatBytes, isImageMime } from '../../../domain/blobUpload';
import { fetchAttachmentBlob, type AttachmentBlob } from '../../driven/attachmentBlobIo';

export type { AttachmentRef, PreparedAttachment } from '../../../domain/attachmentCrypto';
export { parseAttachmentRefs } from '../../../domain/attachments.mjs';
...
function AttachmentCard({ service, ref }: { service: string; ref: AttachmentRef }) {
  const [state, setState] = useState<AttachmentBlob>({ status: 'loading', url: null, bytes: null });
  ...
  useEffect(() => {
    ...
    setRevealed(false);
    void fetchAttachmentBlob(service, ref).then((s) => {
