 ...
 * msgId(16) || body`. The outer sealed box keeps the sender anonymous to the
 * provider; this inner signature lets the *recipient* attribute the message,
 * which is what a 1:1 conversation list needs. The provider never sees it.
 */
const DM_INNER_DOMAIN = new TextEncoder().encode('fly-social-v1/dm-inner');
const DM_INNER_VERSION = 1;
...
function innerSignBytes(fromHex: string, ts: number, msgIdHex: string, body: Uint8Array): Uint8Array {
  const from = hexToBytes(fromHex);
  const msgId = hexToBytes(msgIdHex);
  const time = u64be(ts);
  const out = new Uint8Array(DM_INNER_DOMAIN.length + from.length + time.length + msgId.length + body.length);
  let at = 0;
 ...
 */
export function packDmInner(senderPrivHex: string, body: Uint8Array): PackedDm {
  ...
  const ts = Date.now();
  const sig = bytesToHex(ed25519.sign(innerSignBytes(from, ts, msgId, body), priv));
  const json = JSON.stringify({ v: DM_INNER_VERSION, from, ts, msgId, bodyB64: fromBytes(body), sig });
  return { json, msgId };
}
...
export function unpackDmInner(json: string): OpenedDmInner {
  ...
  const { v, from, ts, msgId, bodyB64, sig } = obj as Record<string, unknown>;
  if (v !== DM_INNER_VERSION) throw new Error('unsupported DM inner version');
  ...
  let ok = false;
  try {
    ok = ed25519.verify(hexToBytes(sig as string), innerSignBytes(normFrom, ts as number, normId, body), hexToBytes(normFrom));
  } catch {
    ok = false;
  }
  if (!ok) throw new Error('DM inner signature invalid');
  return { from: normFrom, ts: ts as number, msgId: normId, body };
}
